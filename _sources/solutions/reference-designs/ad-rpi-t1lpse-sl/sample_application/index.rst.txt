...
3. Connect the first :adi:`AD-SWIOT1L-SL` board to the :adi:`TMP01` temperature
   ...
   .. note::
      ...
      configured as a
      high-impedance voltage input with a 0–5 V range to measure the TMP01 VOUT
      signal.
...
3. Use the firmware images provided in the `pyadi-iio <https://github.com/analogdevicesinc/pyadi-iio/tree/swiot/examples/rpi_t1lpse/host_setup>`_ repository,
   under ``examples/rpi_t1lpse/host_setup/config/firmware``. These images
   configure the boards with static IP addresses:
...
- **Manual static IP** — follow the steps on the AD-RPI-T1LPSE-SL main page
  under :ref:`setting-up-static-ip`.

- **NetworkManager profiles** — use the provided connection profiles and steps
  below (recommended).
...
3. Ensure correct permissions:
   ...
   .. shell::
      ...
      $ sudo chmod 600 /etc/NetworkManager/system-connections/Wired\ connection\
      2
      $ sudo chmod 600 /etc/NetworkManager/system-connections/Wired\ connection\
      3
...
For full host setup details (including the NetworkManager profiles and context),
see the README here:
...
sensor
and compares it against the configured thresholds. The fan is automatically
turned **ON**
once the temperature rises above ``TEMP_ON`` (default 27 °C) and turned **OFF**
once the
temperature falls below ``TEMP_OFF`` (default 26 °C). This hysteresis prevents
rapid
